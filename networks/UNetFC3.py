...
class UnetFC3(nn.Module):
    def __init__(self, conv_dim = 3):
        super(UnetFC3, self).__init__()
        self.e1 = Encoder(3, 64, conv_dim)
        ...
        self.classifier = nn.Sequential(nn.Conv2d(64, out_channels = 3, kernel_size = 1, padding = 0),
            nn.LeakyReLU())

        self.fc = nn.Sequential(nn.Linear(3*224*224+4, 1280),
            nn.LeakyReLU(),
            nn.Linear(1280, 80),
            nn.LeakyReLU(),
            nn.Linear(80, 5),
